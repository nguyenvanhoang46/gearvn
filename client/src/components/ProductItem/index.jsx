            ...
            <div class="bg-white p-4 flex items-center flex-wrap">
              <nav aria-label="Page navigation">
                <ul class="inline-flex">
                  <li onClick={() => changePage(page - 1)}><button class="h-10 px-5 text-green-600 transition-colors duration-150 rounded-l-lg focus:shadow-outline hover:bg-green-100">
                  ...
                  </li>
                  {
                    Array(data?.totalPages ?? 1).fill(0).map((_, index) => {
                      if (index + 1 === Number(page)) {
                        return (
                          <li onClick={() => changePage(index + 1)}><button class="h-10 px-5 text-white transition-colors duration-150 bg-green-600 border border-r-0 border-green-600 focus:shadow-outline">{index + 1}</button></li>
                        )
                      }

                      return (
                        <li onClick={() => changePage(index + 1)}><button class="h-10 px-5 text-green-600 transition-colors duration-150 focus:shadow-outline hover:bg-green-100">{index + 1}</button></li>
                      )
                    })
                  }
                  <li onClick={() => changePage(page + 1)}><button class="h-10 px-5 text-green-600 transition-colors duration-150 bg-white rounded-r-lg focus:shadow-outline hover:bg-green-100">
